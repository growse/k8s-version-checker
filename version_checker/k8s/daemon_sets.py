...
def get_top_level_daemon_sets(namespace: str) -> Dict[Resource, Container]:
    get_deployment_fn, get_pod_fn, get_replica_set_fn, get_stateful_set_fn, get_daemon_set_fn = get_api_functions(
        namespace
    )
    k8s_daemon_set_response = get_daemon_set_fn()
    ...
    return {
        Resource(
            ...
            for pod in get_pods_for_daemon_set(daemon_set, get_pod_fn)
